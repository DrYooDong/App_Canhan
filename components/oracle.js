// ============================================
// NỘI TÂM — Oracle Component (Kỳ Môn, Kinh Dịch & Tri Thức)
// ============================================

(function () {
  'use strict';

  let activeTab = 'compass'; // 'compass' | 'iching' | 'knowledge'

  function renderOracle(container, params) {
    if (params && params[0]) {
      if (['compass', 'iching', 'knowledge', 'journal', 'lessons', 'rules', 'reminders', 'dictionary'].includes(params[0])) {
      ...
      <div class="oracle-container animate-fade-in">
        ...
        <div class="tabs-header" style="display:flex;gap:12px;margin-bottom:24px;border-bottom:1px solid var(--border-color);padding-bottom:12px;flex-wrap:wrap;">
          ...
          <button class="btn btn-tab ${activeTab === 'iching' ? 'active' : ''}" data-tab="iching" style="display:flex;align-items:center;gap:8px;padding:8px 16px;border-radius:8px;font-weight:600;background:${activeTab === 'iching' ? 'var(--accent-muted)' : 'transparent'};color:${activeTab === 'iching' ? 'var(--accent-primary)' : 'var(--text-secondary)'};border:1px solid ${activeTab === 'iching' ? 'var(--border-accent)' : 'transparent'};">
            <span>☯</span> Quẻ Dịch Mai Hoa
          </button>
          <button class="btn btn-tab ${activeTab === 'knowledge' ? 'active' : ''}" data-tab="knowledge" style="display:flex;align-items:center;gap:8px;padding:8px 16px;border-radius:8px;font-weight:600;background:${activeTab === 'knowledge' ? 'var(--accent-muted)' : 'transparent'};color:${activeTab === 'knowledge' ? 'var(--accent-primary)' : 'var(--text-secondary)'};border:1px solid ${activeTab === 'knowledge' ? 'var(--border-accent)' : 'transparent'};">
            <span>📖</span> Tri Thức & Nhật Ký Phản Tư
          </button>
        </div>
    ...
    function loadSubTab(tab) {
      ...
      } else if (tab === 'knowledge' && window.renderKnowledge) {
        window.renderKnowledge(subContent, params && params[0] !== 'knowledge' ? params : []);
      }
    }
